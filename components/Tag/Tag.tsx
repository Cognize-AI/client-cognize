...
type Props = {
  ...
  name: string
  color: string
  setEditTagData: (value: SetStateAction<{
    name: string;
    color: string;
  }>) => void
...
const Tag = ({ id, name, color, setEditTagData, setTagEditing, deleteTag }: Props) => {
  return (
    <div className={styles.tag} style={{ background: color }} key={id}>
      ...
      <Trash onClick={() => deleteTag(id)} className={styles.icons} width={16} height={16} fill='white' />
    </div>
  )
}
